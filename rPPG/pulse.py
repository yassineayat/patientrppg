...
class Pulse():
    ...
    def get_pulse(self, mean_rgb):
        ...
        for t in range(0, (self.signal_size - l + 1)):
            ...
            axs[0].plot(C[0, :], C[1, :], 'o', markersize=3)
            axs[0].set_xlabel('Red')
            axs[0].set_ylabel('Green')
            axs[0].set_title('Original Signal')
            axs[1].plot(P[0, :], P[1, :], 'o', markersize=3)
            axs[1].set_xlabel('Orthogonal Component 1')
            axs[1].set_ylabel('Orthogonal Component 2')
            axs[1].set_title('Signal Projected on Orthogonal Plane')
            plt.tight_layout()
            plt.show()
        return H
