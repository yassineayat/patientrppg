...
class CaptureFrames():
    ...
    def __init__(self, bs, source, show_mask=False):
        ...
        self.model.load_state_dict(torch.load('rPPG/linknet.pth', map_location=self.device))
        self.model.eval()
    ...
    def capture_frames(self, source):
        ...
        while grabbed:
            ...
            # # If no faces are detected, display a message to the user
            # if len(faces) == 0:
            #     cv2.putText(frame, "No visage", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            #
            # # Draw a yellow rectangle around each face
